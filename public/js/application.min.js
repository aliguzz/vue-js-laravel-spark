...
function getSidebarScrollHeight() {
    ...
    if (($nav.hasClass("navbar-fixed-top") && $w.scrollTop() == 0) || $w.scrollTop() == 0)
        height -= 40;
...
$(document).ready(function () {
    ...
    $(".subnav-hidden").each(function () {
        if ($(this).find(".subnav-menu").is(":visible"))
            $(this).find(".subnav-menu").hide();
    });
    ...
    $(".theme-colors > li > span").hover(function (e) {
        ...
        $.ajax({
            url: site_url + "/admin/set_prefrences",
            type: 'post',
            data: {'prefrence_color': "theme-" + $el.attr("class"), '_token': CSRF_TOKEN}
        });
    });
...
$(function () {
    $('.duplicate-lead-btn').click(function () {
        var submitBtn = $(this).next('.duplicateSubmit');
        swal({
...
$(function () {
    $('.mark-form-btn').click(function () {
        var submitBtn = $(this).next('.markSubmit');
        swal({
            title: "Are you sure?",
            text: "You will not be able to recover this record!",
            type: "warning",
...
$(function () {
    $('.approve-form-btn').click(function () {
        var submitBtn = $(this).next('.approveSubmit');
        swal({
            title: "Approve User",
...
$('.upgrade-button').click(function () {
    var submitBtn = $(this).next('.upgradeSubmit');
    swal({
        title: "Upgrade Package",
        text: "Are you sure you want to upgrade your package?",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Yes, Package Upgraded!",
        cancelButtonText: "No, cancel!",
        closeOnConfirm: false,
        closeOnCancel: true
    },
...
$(document).on('change', '.load_lead_types', function () {
    $("#small-ui-block-loader").show();
    $.ajax({
        url: site_url + "/admin/get_lead_types",
        type: 'post',
        data: {'lead_group_id': $(this).val(), 'edit_id': $('#edit_id').val(), '_token': CSRF_TOKEN},
        success: function (data) {
            $("#lead_type_div").html(data);
            $("#lead_type_div").removeClass('reverse-form-group');
            $("#small-ui-block-loader").hide();
        }
    });
});
...
$(document).ready(function () {
    ...
    $("#left .subnav-menu").each(function () {
        var element = $(this);
        if ($.trim(element.html()) == '') {
            element.parent('.subnav').remove();
        }
    });
    /*    setTimeout(function () {
     var parent_style = $("#left .slimScrollDiv").attr('style');
     var child_style = $("#left .scrollable").attr('style');
     
     $("#left .slimScrollDiv").attr('style', parent_style.replace('px', '%'));
     $("#left .scrollable").attr('style', child_style.replace('px', '%'));
